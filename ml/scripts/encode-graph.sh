...
parameters=("--strict" "--deduplicate" "--continue-on-error" "--raw-strings" "--only-encoded-features" "--relationships-as-edges" "true" "--only-containment-associations" "false" "--edge-tag-as-attribute" "true")
