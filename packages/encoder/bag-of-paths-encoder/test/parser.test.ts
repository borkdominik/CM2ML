...
describe('parser', () => {
  it('parses keyword templates', () => {
    ...
    expect(template(testModel.root, { length: 0, step: 0 })).toBe('root with type node and tag root')
    expect(template(testModel.getNodeById('a')!, { length: 0, step: 0 })).toBe('a with type node and tag node')
  })
  ...
  describe('conditions', () => {
    ...
    it('parses conditional templates', () => {
      ...
      expect(template(testModel.root, { length: 0, step: 0 })).toBe('the root of type node')
      expect(template(testModel.getNodeById('a')!, { length: 0, step: 0 })).toBe(undefined)
    })

    describe('path selector', () => {
      it('parses step matches', () => {
        const template = compileNodeTemplate('@path.step >= 1 -> not the first step')
        expect(template(testModel.root, { length: 0, step: 0 })).toBe(undefined)
        expect(template(testModel.root, { length: 2, step: 1 })).toBe('not the first step')
        expect(template(testModel.getNodeById('a')!, { length: 2, step: 1 })).toBe('not the first step')
      })

      it('parses length matches', () => {
        const template = compileNodeTemplate('@path.length >= 1 -> at least one step')
        expect(template(testModel.root, { length: 0, step: 0 })).toBe(undefined)
        expect(template(testModel.root, { length: 1, step: 1 })).toBe('at least one step')
        expect(template(testModel.root, { length: 2, step: 1 })).toBe('at least one step')
      })
    })
    ...
    describe('comparison operators', () => {
      describe('for numbers', () => {
        it('parses equality', () => {
          ...
          expect(template(testModel.root, { length: 2, step: 0 })).toBe('only the first')
          expect(template(testModel.root, { length: 2, step: 1 })).toBe(undefined)
        })

        it('parses inequality', () => {
          const template = compileNodeTemplate('@path.step != 0 -> not the first')
          expect(template(testModel.root, { length: 2, step: 0 })).toBe(undefined)
          expect(template(testModel.root, { length: 2, step: 1 })).toBe('not the first')
        })

        it('parses less than', () => {
          const template = compileNodeTemplate('@path.step < 1 -> the first')
          expect(template(testModel.root, { length: 2, step: 0 })).toBe('the first')
          expect(template(testModel.root, { length: 2, step: 1 })).toBe(undefined)
          expect(template(testModel.root, { length: 2, step: 2 })).toBe(undefined)
        })

        it('parses less than or equal', () => {
          const template = compileNodeTemplate('@path.step <= 1 -> the first or second')
          expect(template(testModel.root, { length: 2, step: 0 })).toBe('the first or second')
          expect(template(testModel.root, { length: 2, step: 1 })).toBe('the first or second')
          expect(template(testModel.root, { length: 2, step: 2 })).toBe(undefined)
        })
        ...
        it('parses greater than or equal', () => {
          ...
          expect(template(testModel.root, { length: 2, step: 0 })).toBe(undefined)
          expect(template(testModel.root, { length: 2, step: 1 })).toBe('not the first')
          expect(template(testModel.root, { length: 2, step: 2 })).toBe('not the first')
        })
      })
      ...
      describe('for strings', () => {
        it('parses equality', () => {
          ...
          expect(template(testModel.root, { length: 0, step: 0 })).toBe('the root')
          expect(template(testModel.getNodeById('a')!, { length: 0, step: 0 })).toBe(undefined)
        })
        ...
        it('parses inequality', () => {
          ...
          expect(template(testModel.root, { length: 0, step: 0 })).toBe(undefined)
          expect(template(testModel.getNodeById('a')!, { length: 0, step: 0 })).toBe('not the root')
        })

        it('parses less than', () => {
          const template = compileNodeTemplate('@id< aa -> before aa')
          expect(template(testModel.getNodeById('a')!, { length: 0, step: 0 })).toBe('before aa')
          expect(template(testModel.getNodeById('aa')!, { length: 0, step: 0 })).toBe(undefined)
          expect(template(testModel.getNodeById('b')!, { length: 0, step: 0 })).toBe(undefined)
        })

        it('parses less than or equal', () => {
          const template = compileNodeTemplate('@id <= aa -> before or equal to aa')
          expect(template(testModel.getNodeById('a')!, { length: 0, step: 0 })).toBe('before or equal to aa')
          expect(template(testModel.getNodeById('aa')!, { length: 0, step: 0 })).toBe('before or equal to aa')
          expect(template(testModel.getNodeById('b')!, { length: 0, step: 0 })).toBe(undefined)
        })
        ...
        it('parses greater than', () => {
          ...
          expect(template(testModel.getNodeById('a')!, { length: 0, step: 0 })).toBe(undefined)
          expect(template(testModel.getNodeById('aa')!, { length: 0, step: 0 })).toBe(undefined)
          expect(template(testModel.getNodeById('b')!, { length: 0, step: 0 })).toBe('after aa')
        })

        it('parses greater than or equal', () => {
          const template = compileNodeTemplate('@id >= aa -> after or equal to aa')
          expect(template(testModel.getNodeById('a')!, { length: 0, step: 0 })).toBe(undefined)
          expect(template(testModel.getNodeById('aa')!, { length: 0, step: 0 })).toBe('after or equal to aa')
          expect(template(testModel.getNodeById('b')!, { length: 0, step: 0 })).toBe('after or equal to aa')
        })
      })
    })
  })
