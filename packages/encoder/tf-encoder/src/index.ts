...
import { batchTryCatch, compose, definePlugin, ExecutionError } from '@cm2ml/plugin'
...
const TermFrequencyExtractor = definePlugin({
    ...
    invoke(batch: InferOut<typeof TermExtractor>, parameters) { 
        ...
        const documentFrequency: number[] = new Array(termList.length).fill(0)
        const termDocumentMatrix: TermDocumentMatrix = {}

        validItems.forEach(item => {
            const docVector = new Array(termList.length).fill(0)
